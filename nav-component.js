...
(function() {
    ...
    // Skip nav if loaded inside an iframe (e.g., from suitegpt.app app preview)
    if (window.self !== window.top) {
        nav.style.display = 'none';
        // Also remove top padding from body since nav is hidden
        ...
        <div class="connect-modal-overlay" id="navConnectModalOverlay" onclick="if(event.target === this) closeNavConnectModal()">
            <div class="connect-modal">
                ...
                <div class="connect-options">
                    <div class="connect-option google" onclick="loginWithGoogleFromNav()">
                        ...
                        <div>
                            <div class="connect-option-title">Continue with Google</div>
                            <div class="connect-option-desc">Quick sign in with your Google account</div>
                        </div>
                    </div>
    ...
    async function getGoogleUser() {
        ...
        try {
            ...
            console.error('[Nav Auth] Error checking Google session:', err);
        }
        return null;
    }
    ...
    async function updateNavAuthDisplay() {
        ...
        if (wallet) {
            ...
            connectBtn.style.display = 'none';
            authDisplay.style.display = 'flex';
            identityEl.textContent = googleUser.name || googleUser.email?.split('@')[0] || 'User';
        ...
        console.log('[Nav Auth] State:', { wallet: !!wallet, googleUser: !!googleUser, tgUser: !!tgUser });
    }
...
async function loginWithGoogleFromNav() {
    closeNavConnectModal();
...
async function startGoogleOAuth(supabaseClient) {
    try {
        ...
        if (error) {
            console.error('[Nav Auth] Google OAuth error:', error);
            alert('Failed to start Google sign-in: ' + error.message);
        }
    } catch (err) {
        console.error('[Nav Auth] Google OAuth exception:', err);
...
async function openNavProfileMenu() {
    ...
    if (window.supabase) {
        try {
            ...
            console.error('[Nav] Error checking Google session:', err);
        }
    }
    ...
    const truncateEmail = (email) => email?.length > 20 ? email.slice(0, 17) + '...' : email;
    ...
    if (googleUser) {
        content += `
            <div class="connect-option connected">
                ...
                <div style="flex:1">
                    <div class="connect-option-title">Google Connected</div>
                    <div class="connect-option-desc">${truncateEmail(googleUser.email)}</div>
                </div>
                <button class="disconnect-btn" onclick="disconnectGoogle()">Disconnect</button>
            </div>
        `;
    } else {
        content += `
            <div class="connect-option google" onclick="loginWithGoogleFromNav()">
                ...
                <div>
                    <div class="connect-option-title">Connect Google</div>
                    <div class="connect-option-desc">Link your Google account</div>
                </div>
            </div>
        `;
    }
...
async function disconnectGoogle() {
    ...
    if (window.supabase) {
        try {
            ...
            console.error('[Nav] Error signing out of Google:', err);
        }
    }
    closeNavConnectModal();
    if (window.refreshNavCredits) {
