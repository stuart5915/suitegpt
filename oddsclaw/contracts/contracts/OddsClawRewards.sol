...
/// @title OddsClawRewards — user-claimable ODDS reward distribution
...
contract OddsClawRewards is ReentrancyGuard {
    ...
    uint256 public totalDeposited;      // total ODDS loaded into contract
    uint256 public totalDistributed;    // total ODDS sent to players
    ...
    constructor(address _oddsToken, address _signer) {
    ...
    /// @notice Deposit ODDS tokens into the reward pool
    function depositRewards(uint256 amount) external onlyAdmin nonReentrant {
        require(amount > 0, "Zero amount");
        oddsToken.safeTransferFrom(msg.sender, address(this), amount);
    ...
    function adminWithdraw(uint256 amount) external onlyAdmin nonReentrant {
        require(amount > 0, "Zero amount");
        require(oddsToken.balanceOf(address(this)) >= amount, "Insufficient balance");
        oddsToken.safeTransfer(admin, amount);
    ...
    /// @notice Claim ODDS rewards — user pays gas, server signs authorization
    ...
    function claim(uint256 amount, uint256 nonce, bytes calldata signature) external nonReentrant whenNotPaused {
        ...
        require(oddsToken.balanceOf(address(this)) >= amount, "Insufficient reward pool");
        ...
        oddsToken.safeTransfer(msg.sender, amount);
    ...
    function rewardStats() external view returns (
        ...
        return (totalDeposited, totalDistributed, oddsToken.balanceOf(address(this)));
    }
}
