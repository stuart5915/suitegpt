...
function statRow(label, value, valueColor) {
    return h('div', {
        style: {
            ...
            alignItems: 'center', marginBottom: 18, fontSize: 38,
        }
    },
...
function smallRow(label, value) {
    return h('div', {
        style: {
            display: 'flex', justifyContent: 'space-between',
            alignItems: 'center', marginBottom: 8, fontSize: 30,
        }
    },
        h('span', { style: { color: COLORS.muted } }, label),
...
function cardLabel(text) {
    return h('div', {
        style: { display: 'flex', fontSize: 28, color: COLORS.muted, marginBottom: 8, letterSpacing: 2 }
    }, text.toUpperCase());
}

function cardValue(text, extraStyle) {
    return h('div', {
        style: { display: 'flex', fontSize: 58, fontWeight: 700, color: COLORS.text, ...(extraStyle || {}) }
    }, String(text));
}

function cardTitle(text) {
    return h('div', {
        style: { display: 'flex', fontSize: 40, fontWeight: 700, marginBottom: 24, color: COLORS.text }
    }, text);
}
    ...
    try {
        ...
        const element = h('div', {
            style: {
                ...
                backgroundColor: COLORS.bg, padding: 70,
            ...
            h('div', {
                ...
                h('div', { style: { display: 'flex', alignItems: 'center', fontSize: 64, fontWeight: 700 } },
                    '$CLAWS Daily'
                ),
                h('div', { style: { display: 'flex', fontSize: 42, color: COLORS.muted } }, p('date', 'Today'))
            ),

            // ── Top row: Price / Treasury (with breakdown) / Incubations ──
            h('div', { style: { display: 'flex', marginBottom: 30 } },
                // Price
                card([
                    cardLabel('Price'),
                    cardValue(p('price', '—')),
                    h('div', { style: { display: 'flex', fontSize: 34, color: changeColor, marginTop: 4 } }, changeText + ' today'),
                ], { marginRight: 24, flex: 0.8 }),

                // Treasury with breakdown
                card([
                    cardLabel('Treasury'),
                    h('div', { style: { display: 'flex', alignItems: 'center' } },
                        cardValue(p('treasury', '—'), { marginRight: 16 }),
                        treasuryDelta ? h('span', { style: { fontSize: 32, color: treasuryDeltaColor } }, '(' + treasuryDelta + ')') : null
                    ),
                ...
                ], { marginRight: 24, flex: 1.4 }),

                // Incubations
                card([
                    cardLabel('Incubations'),
                    cardValue(p('incubations', '0')),
                    h('div', { style: { display: 'flex', fontSize: 34, color: COLORS.muted, marginTop: 4 } }, 'active'),
                ], { flex: 0.6 })
            ),
            ...
            h('div', {
                style: {
                    ...
                    borderTop: '2px solid #2a2a3e', fontSize: 32,
                }
            },
                h('span', null, p('locked', '—') + '% locked'),
                p('votes') ? h('span', { style: { color: COLORS.muted, fontSize: 26 } },
