...
interface CombineImagesRequest {
    postId: string
    platform: string
    content: string
    templateId?: string
...
export async function POST(req: NextRequest) {
    console.log('[combine-images] API called')
    try {
        const body: CombineImagesRequest = await req.json()
        const { postId, platform, content, templateId, aiBackgroundUrl } = body
        console.log('[combine-images] Request:', { postId, platform, contentLength: content?.length, templateId, aiBackgroundUrl: aiBackgroundUrl?.substring(0, 50) })

        if (!postId || !platform || !content || !aiBackgroundUrl) {
            console.log('[combine-images] Missing fields')
            return NextResponse.json(
                { error: 'Missing required fields: postId, platform, content, aiBackgroundUrl' },
                { status: 400 }
            )
        }
        ...
        console.log('[combine-images] Selected template:', template.id, template.name)
        ...
        console.log('[combine-images] Headline:', headline)
        console.log('[combine-images] Subheadline:', subheadline)

        // Generate the combined image using next/og with AI background
        ...
        console.log('[combine-images] Converting ImageResponse to buffer...')
        const imageBuffer = await imageResponse.arrayBuffer()
        console.log('[combine-images] Buffer size:', imageBuffer.byteLength)
        const base64Image = Buffer.from(imageBuffer).toString('base64')

        // Upload to Supabase storage
        const fileName = `post-images/combined-${postId}-${Date.now()}.png`
        console.log('[combine-images] Uploading to:', fileName)
        let imageUrl: string
        ...
        // Update the post with both combined_image_url and images array
        await fetch(
        ...
        return NextResponse.json(
            { error: error instanceof Error ? error.message : 'Failed to combine images' },
            { status: 500 }
        )
    }
}
